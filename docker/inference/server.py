...
model = "/app/models/iris_model.pkl"
...
def predict():
    ...
    iris_input = np.array(iris_input).reshape(1, -1)  #  use np.array().reshape(1, -1) to convert input to 2D array
    prediction = joblib.load(model).predict(iris_input)
